...
CKObjectDeclaration *FillBehaviorSetDestBlendDecl();
CKERROR CreateSetDestBlendProto(CKBehaviorPrototype **);
int SetDestBlend(const CKBehaviorContext &behcontext);

CKObjectDeclaration *FillBehaviorSetDestBlendDecl()
{
    CKObjectDeclaration *od = CreateCKObjectDeclaration("Set Dest Blend");
    od->SetDescription("Sets the Destination Mode when Blending is used on the material of the 3D entity.");
    /* rem:
    ...
    */
    ...
    od->SetGuid(CKGUID(0x036d1a87, 0xdf0a670c));
    od->SetAuthorGuid(VIRTOOLS_GUID);
    od->SetAuthorName("Virtools");
    od->SetVersion(0x00010000);
    od->SetCreationFunction(CreateSetDestBlendProto);
...
CKERROR CreateSetDestBlendProto(CKBehaviorPrototype **pproto)
{
    CKBehaviorPrototype *proto = CreateCKBehaviorPrototype("Set Dest Blend");
    if (!proto)
    ...
    proto->DeclareInParameter("Blending Mode", CKPGUID_BLENDFACTOR, "Zero");

    proto->SetFlags(CK_BEHAVIORPROTOTYPE_OBSOLETE);
    proto->SetFunction(SetDestBlend);
...
int SetDestBlend(const CKBehaviorContext &behcontext)
{
    ...
    int mode = VXBLEND_ZERO;
    beh->GetInputParameterValue(0, &mode);

    mat->SetDestBlend((VXBLEND_MODE)mode);
