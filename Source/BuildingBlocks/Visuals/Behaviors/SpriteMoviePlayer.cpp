...
CKObjectDeclaration *FillBehaviorSpriteMoviePlayerDecl();
CKERROR CreateSpriteMoviePlayerProto(CKBehaviorPrototype **pproto);
int SpriteMoviePlayer(const CKBehaviorContext &behcontext);

CKObjectDeclaration *FillBehaviorSpriteMoviePlayerDecl()
{
    CKObjectDeclaration *od = CreateCKObjectDeclaration("Sprite Movie Player");
    od->SetDescription("Plays a movie in a 2d Sprite (avi)");
    /* rem:
    <SPAN CLASS=in>On: </SPAN>activates the process.<BR>
    <SPAN CLASS=in>Off: </SPAN>deactivates the process.<BR>
    <BR>
    <SPAN CLASS=out>Slot Complete: </SPAN>activates each time a slot change.<BR>
    <SPAN CLASS=out>Movie Complete: </SPAN>activates each time a sequence has been completed.<BR>
    <BR>
    ...
    */
    ...
    od->SetGuid(CKGUID(0x4c1d16ac, 0x1877604a));
    od->SetAuthorGuid(VIRTOOLS_GUID);
    od->SetAuthorName("Virtools");
    od->SetVersion(0x00010000);
    od->SetCreationFunction(CreateSpriteMoviePlayerProto);
...
CKERROR CreateSpriteMoviePlayerProto(CKBehaviorPrototype **pproto)
{
    CKBehaviorPrototype *proto = CreateCKBehaviorPrototype("Sprite Movie Player");
    if (!proto)
    ...
    proto->DeclareOutput("Slot Complete");
    proto->DeclareOutput("Movie Complete");
    ...
    proto->DeclareLocalParameter(NULL, CKPGUID_FLOAT); // Current Time

    proto->SetFlags(CK_BEHAVIORPROTOTYPE_NORMAL);
    proto->SetFunction(SpriteMoviePlayer);
...
int SpriteMoviePlayer(const CKBehaviorContext &behcontext)
{
    ...
    { // it's the first time we activate the behavior
    ...
    if (current_time >= fduration)
    {                                 // If we exceed the total duration
        ...
        if (b)
        { // if we ask to loop then loop
            do
                current_time -= fduration;
            while (current_time >= fduration);
        }
        else
        { // if we ask not to loop then stop playing
            return CKBR_OK;
        }
    }
    ...
    { // we change the slot only if we need to.
